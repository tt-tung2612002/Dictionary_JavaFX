...
public class DatabaseManager {
	...
	public String getFormattedResult(String searchedWord, DictionaryType dict) throws SQLException {
		...
		for (int i = 0; i < ans.size(); i++) {
			...
			examples.add(example != null ? Arrays.asList(example.split(",")) : null);
			synonyms.add(synonym != null ? Arrays.asList(synonym.split(",")) : null);
		}
		String pronunciation = server2.getPronounciation(searchedWord);
		String formattedResult = resultFormatter(word, types, pronunciation, meanings, examples, synonyms, adjTypes);
		return formattedResult;

	}

	public String resultFormatter(String word, List<String> types, String pronunciation, List<String> meanings,
			List<List<String>> exampless, List<List<String>> synonymss, List<String> adjTypes) {
		String images = "<div style=\"position: absolute; top: 15px; left: 700px;\"><img src=\"https://i.ibb.co/L5ykRV0/facebook.png\"\r\n"
		...
		String formatted = "<!DOCTYPE html><html lang='en' style='background-color: white;font-size:21px; height: 610px; width:660; font-family: Arial, Helvetica, sans-serif;'><head><meta charset='UTF-8'><meta http-equiv='X-UA-Compatible' content='IE=edge'><meta name='viewport' content='width=device-width, initial-scale=1.0'><title>Tu dien</title><style>\r\n"
				+ "        ol li {\r\n" + "            font-weight: bold;\r\n" + "        }\r\n" + "\r\n"
				+ "        li>p {\r\n" + "            font-weight: normal;\r\n" + "        }\r\n" + "\r\n"
				+ "        ol li>ul>Li {\r\n" + "            font-weight: normal;\r\n" + "        }\r\n"
				+ "    </style></head><body style='color: #1d2a57; margin:0px 5px; padding: 0px;'>" + images
				...
				+ word + "</b> trong Tiếng " + (dictType == DictionaryType.ENG_ENG ? "Anh" : "Việt")
				...
				+ word + "</b></p>" + "<p style='margin: 5px 0; font-size: 20px;'><b>" + pronunciation + "</b></p>"
		...
		formatted += "<p style=\"margin: 5px 5px; font-size: 24px;\"><b>" + currentType + "</b></p>\r\n"
				+ "            <ol>";
		for (int i = 0; i < types.size(); i++) {
			if (types != null) {
				if (types.get(i) != null) {
					...
					if (!type.equals(currentType)) {
						currentType = type;
						formatted += "</ol> <div style=\"line-height: 300%;padding: 0px 15px; border-bottom: 3px solid #fdc702;\">\r\n"
								+ "\r\n" + "            </div>" + "<p style=\"margin: 5px 0; font-size: 24px;\"><b>"
								+ type + "</b></p>\r\n" + "            <ol>";
					}
				}
			}

			formatted += "<li style=\"margin: 0px 5px 5px 15px; font-size: 20px;list-style-type:decimal;\">"
			...
			synonym = synonym.length() == 0 ? "" : "(" + synonym.substring(0, synonym.length() - 2) + ") ";
			formatted += synonym + meaningFormatter(meanings.get(i))
					+ (adjTypes != null && adjTypes.get(i) != null ? " (" + adjTypes.get(i) + ")" : "") + "</h3>\r\n"
				...
				if (exampless.get(i) != null) {
					...
					if (examples != null) {
						for (String example : examples) {
							formatted += "<li style='font-style: italic;margin: 5px 5px 10px 5px; font-size: 22px; list-style-type: disc;'><i>"
	...
	public String getResultFromFile(String searched) throws SQLException {
		...
		List<List<String>> exampless = dictionaryData.getWordExampless().get(pos);
		String pronounciation = server2.getPronounciation(searched);

		return resultFormatter(word, types, pronounciation, meanings, exampless, null, null);
	}
	...
	public String getResultFromAPI(String searched) throws SQLException {
		...
		String pronounciation = server2.getPronounciation(searched);
		String formattedResult = resultFormatter(searched, types, pronounciation, meanings, examples, null, adjTypes);
		return formattedResult;
	}

	public String translate(String langFrom, String langTo, String text) throws IOException {
		String urlStr = "https://script.google.com/macros/s/AKfycby1KXAnwL5Nm1WY3m_CZ4BENAfQZCqcQ8Vyzcam8TEIsu9pL0DUz_E55nzYGSy0Av795g/exec"
